...
class FootstepNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算期望的元素数
        expected_size = 15 + 2 + OBSERVATION_SIZE * OBSERVATION_SIZE
        batch_size = x.size(0)
        expected_total = batch_size * expected_size

        # 检查输入大小
        if x.numel() != expected_total:
            print(f"警告: 输入大小不匹配! 期望: {expected_total}, 实际: {x.numel()}")
            # 调整输入大小
            x = x[:, :expected_size]
            print(f"调整后输入形状: {x.shape}")
        ...
        # 检查观测区域大小
        obs_size = obs_flat.size(1)
        expected_obs_size = OBSERVATION_SIZE * OBSERVATION_SIZE
        if obs_size != expected_obs_size:
            print(f"警告: 观测区域大小不匹配! 期望: {expected_obs_size}, 实际: {obs_size}")
            # 调整观测区域大小
            obs_flat = obs_flat[:, :expected_obs_size]
        ...
        try:
            ...
            print(f"重塑观测区域时出错: {e}")
            print(f"观测区域形状: {obs_flat.shape}")
            print(f"尝试重塑为: (-1, 1, {OBSERVATION_SIZE}, {OBSERVATION_SIZE})")
            # 计算实际元素数
            actual_elements = obs_flat.numel()
            # 计算可以重塑的大小
            new_size = int(np.sqrt(actual_elements // batch_size))
            if new_size > 0:
                print(f"尝试重塑为 {new_size}x{new_size}")
                obs_2d = obs_flat.view(-1, 1, new_size, new_size)
            else:
                # 如果无法重塑，使用零张量
                print("使用零张量替代")
                obs_2d = torch.zeros(batch_size, 1, OBSERVATION_SIZE, OBSERVATION_SIZE, device=x.device)
...
def prepare_state_tensor(state_dict,height_map):
    ...
    if obs_region.size != OBSERVATION_SIZE * OBSERVATION_SIZE:
        # 如果大小不正确，调整为正确大小
        obs_region = np.resize(obs_region, (OBSERVATION_SIZE, OBSERVATION_SIZE))
        print("prepare_state_tensor:WARNING! obs_region is not right! resized")
    obs_flat = obs_region.flatten()
    ...
    # 动作部分 - 这里需要实际的动作值
    # 但在状态准备时，我们还没有动作，所以使用零向量作为占位符
    action_vector = [0, 0]  # 占位符
    ...
    if len(input_vector) != expected_size:
        # 如果大小不正确，调整为正确大小
        input_vector = np.resize(input_vector, expected_size)
        print("prepare_state_tensor:WARNING! input_tensor size error,now resized")
...
def train_step(model, target_model, optimizer, states, actions, rewards, next_states, dones, height_map, gamma):
    ...
    for i in range(len(states)):
        # 准备状态张量
        state_tensor = prepare_state_tensor(states[i], height_map)
        ...
        input_tensors.append(input_vector)
    ...
    # 检查输入张量形状
    print(f"输入张量形状: {input_tensors.shape}")

    # 计算当前Q值
    current_q = model(input_tensors).squeeze(1)

    # 准备下一状态张量
    next_state_tensors = []
    for next_state in next_states:
        next_state_tensor = prepare_state_tensor(next_state, height_map)
        # 添加零动作占位符
        zero_action = torch.zeros(2, dtype=torch.float32)
        next_input = torch.cat([next_state_tensor, zero_action])
        next_state_tensors.append(next_input)
    ...
    with torch.no_grad():
        ...
        # 使用目标网络评估这些动作
        next_q = target_model(next_state_tensors).gather(1, next_actions_idx.unsqueeze(1)).squeeze(1)
...
def train_rl_agent(env, model,height_map, num_episodes=1000, batch_size=64, gamma=0.99):
    """
    训练强化学习代理
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            if np.random.rand() < epsilon:
                ...
                with torch.no_grad():
                    # 准备状态张量
                    state_tensor = prepare_state_tensor(state,height_map)
                    # 预测Q值
