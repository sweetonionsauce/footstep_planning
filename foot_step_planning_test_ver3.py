...
class FootstepNet(nn.Module):
    ...
    def forward(self, state, obs, action):
        ...
        输入:
            ...
            obs: 观测区域展平 [batch_size, OBSERVATION_SIZE*OBSERVATION_SIZE]
        ...
        # 处理观测区域 (恢复为2D)
        obs_2d = obs.view(-1, 1, OBSERVATION_SIZE, OBSERVATION_SIZE)
...
def prepare_state_tensor(state_dict, height_map):
    ...
    # 返回状态向量和观测区域
    return torch.tensor(state_vector, dtype=torch.float32), torch.tensor(obs_flat, dtype=torch.float32)
...
def train_step(model, target_model, optimizer, states, actions, rewards, next_states, dones, height_map, gamma):
    ...
    for next_state in next_states:
        next_state_tensor, next_obs_tensor = prepare_state_tensor(next_state, height_map)
    ...
    with torch.no_grad():
        ...
        next_q_values = model(next_state_tensors, next_obs_tensors, action_tensors)
