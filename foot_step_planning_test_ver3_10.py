    ...
    for episode in range(num_episodes):
        ...
        while not done and step_count < MAX_STEPS_PER_EPISODE:
            ...
            if step_count % 10 == 0:
                ...
                print(
                    f"Episode {episode}, Step {step_count}, 耗时: {elapsed:.4f}s, episode总耗时: {episode_elapsed:.2f}s")
            step_count += 1
        ...
        if total_reward > best_reward:
            ...
            torch.save(model.state_dict(), f'best_model_ep{episode}.pth')

        # 收敛检测
        if episode > 20:  # 至少20回合后开始检测
            # 1. 损失稳定性检测
            ...
            reward_change = abs(recent_rewards[-1] - recent_rewards[0]) / abs(recent_rewards[0] + 1e-5)

            if loss_std < 0.001 and reward_change < 0.05:
                convergence_count += 1
            else:
                convergence_count = 0

            if convergence_count >= 3:
                print(f"模型已收敛！在回合 {episode} 停止训练")
                break

        # 打印进度
        if episode % 1 == 0:  # 每回合都打印
...
def evaluate_model(env, model, num_episodes=10, render=False):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            # 使用模型选择动作
            # 准备状态张量和观测张量
            state_tensor, obs_tensor = prepare_state_tensor(state, height_map=I_noise)

            # 创建动作占位符（零向量）
            action_placeholder = torch.zeros(2, dtype=torch.float32)

            with torch.no_grad():
                # 扩展维度以匹配模型输入要求（batch维度）
                state_tensor = state_tensor.unsqueeze(0)
                obs_tensor = obs_tensor.unsqueeze(0)
                action_placeholder = action_placeholder.unsqueeze(0)

                # 预测Q值
                q_values = model(state_tensor, obs_tensor, action_placeholder)
                action_idx = q_values.argmax().item()
...
if __name__ == "__main__":
    ...
    if os.path.exists('footstep_planning_model.pth'):
        ...
        # 训练模型
    print("开始训练强化学习代理...")
    episode_rewards, episode_losses = train_rl_agent(env, model, num_episodes=5, height_map=I_noise)
